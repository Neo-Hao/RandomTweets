...
### bring in the data
# load data
data <- read.csv("total.csv", header = T, stringsAsFactors = F)
# write.csv(as.matrix(data$class), file="class.csv", row.names = F)
data <- data$text
len <- length(data)
...
###### get class
total <- read.csv("total.csv", header = T, stringsAsFactors = F)
total <- total[empty,]
total <- total$class
write.csv(as.matrix(total), file="class.csv", row.names = F)
...
### tf-idf
write.csv(as.matrix(dtm), file="tf-idf.csv", row.names = F)
